    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        if(!isset($_SESSION["employeeCredentials"])){
            ...
            if (isset($_POST['login']) && !empty($_POST['mailAddress']) && !empty($_POST['password'])) {
                    ...
                    if($stop == false){
                        $result = checkEmployeesCredentials($mailAddress,$password);
                        if($result != "invalid credentials"){
                            $_SESSION["employeeCredentials"] = $result;
                        }
                        print_r($_SESSION["employeeCredentials"]);
                        header("Location: homeworks.php");
                    }
            }
        }else{
            header("Location: homeworksAdmin.php");
            exit;
        }
    }
?>
